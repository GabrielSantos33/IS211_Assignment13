"""Gabriel Santos IS-211 11/14/2020"""
    ...
    if request.method == 'POST':
        if request.form['quiz_subject'] is not None:
            try:
                ...
                con.row_factory = sqlite3.Row
                cur = con.cursor()
                cur.execute("INSERT INTO quizzes (quiz_id, quiz_subject, quiz_question_amount, quiz_date) VALUES ({}, {}, {}, {})".format(
                ...
                error = 'Invalid Name. Please try again.'
                return render_template('score_add.html', error=error)
        else:
            return redirect(url_for('dashboard'))
    else:
...
def score_add():
    ...
    if request.method == 'POST':
        ...
        return render_template("score_add.html", quizrows=quizrows, studentrows=studentrows)


@app.route('/student/<int_sid>', methods=['GET', 'POST'])
def student_id_pass(int_sid):
